...
class AuthController extends Controller
{
    ...
    public function registerFromQq()
    {
        if (!session()->has('qqData')) {
            return redirect('login');
        }
        $githubData = array_merge(session('qqData'), request()->old());
        return view('auth.qq_register', compact('qqData'));
    }
    ...
    {
        $githubUser = Socialite::driver('qq')->user();
        $user = User::where('github_id', $githubUser->id)->first();
